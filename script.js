...
const tellMeAJoke = (joke) => {
    VoiceRSS.speech({
        key: 'df3d6f91460e4a79a5729366cb5eb5ba',
        src: joke,
        hl: 'en-gb',
        v: 'Alice',
        r: 0, 
...
async function getJokeFromAPI(){
    ...
    try {
        ...
        if (data.type === 'single') {                        
            joke = data.joke;                               //single joke
        } else {
            joke = `${data.setup} ... ${data.delivery}`;    //two part joke
        }
        tellMeAJoke(joke);
        toggleButton();
    } catch (error) {
        console.log('Ops, no jokes!', error);
    }
}
